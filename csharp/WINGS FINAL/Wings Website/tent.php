...
$tent_query = mysql_query("select * from `tents`");
if($my_query=mysql_query("select * from `camping` where Customer_Id=$userid"))
{
echo'<h1>My Tent Info</h1>';
echo '<hr width="600px" size="2">';

while($myrow=mysql_fetch_array($my_query))
{$tempid = $myrow['Hoster'];
$getsql = mysql_query("select * from `user_reservation` where Customer_Id=$tempid limit 1");
$client_row = mysql_fetch_array($getsql);
if($client_row['User_Name']==$username)
{
echo 'You have booked ',' tent No: ',$myrow['Tent_Number'],'.<br / >';

}
else 
{
echo 'You have been invited by ',$client_row['User_Name'],' in tent No: ',$myrow['Tent_Number'],'.<br / >';
}
}

}

echo '<h1>Tent List</h1><br />';
echo '<hr width="600px" size="2">';
$tentNo = 0;
while($row = mysql_fetch_array($tent_query))
{
if($row['Isfull']==0){
$tentNo = $row['Tent_Number'];
echo 'Tent Number:',$tentNo,'<br />';
echo 'Number of people:',$row['No_Of_People'],'<br />';
echo 'Location:',$row['Location'],'<br />';
echo 'Rent:',$row['Rent'],'<br />';
echo '<a href="reserveconfirm.php?tentNo=',$tentNo,'">Reserve</a><br/>';
echo '<hr width="600px" size="2">';

}else{
echo 'Tent Number:',$row['Tent_Number'],'<br />';
echo 'Number of people:',$row['No_Of_People'],'<br />';
echo 'Location:',$row['Location'],'<br />';
echo 'Rent:',$row['Rent'],'<br />';
echo 'Full<br/>';
...
echo' <a href="my.html">back to my pofile</a>';
