...
//检测用户名是否已经买票
$check_query = mysql_query("select Customer_Id from `user_confirmation` where Customer_Id='$userid' limit 1");
if(mysql_fetch_array($check_query)){
    echo 'Error:User ',$username,' have already order the ticket.<a href="my.html">back</a>';
    exit;
}
//写入数据
$acc_query = mysql_query("select * from `account` where Customer_Id=$userid limit 1");
$row = mysql_fetch_array($acc_query);
$mia = $row['Money_In_Account'];
if($mia>=55){
$mia-=55;
$buysql = "UPDATE `account` SET  Money_In_Account = $mia WHERE Customer_Id=$userid";
if(mysql_query($buysql,$conn)){
$sql = "INSERT INTO `user_confirmation`(Customer_Id)
VALUES('$userid')";
if(mysql_query($sql,$conn)){
    exit('Well done! <a href="my.html">Bcak to my profile!</a>');
} else {
    echo 'Sorry!Here is some error!',mysql_error(),'<br />';
    echo ' <a href="my.html">Back</a> and try again!';
}
}
}else{echo 'Sorry!You dont have enough money!<br />';
echo ' <a href="tf.html">Here to transfer money!</a>';}
